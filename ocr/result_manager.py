...
class ResultManager:
    ...
    def start(self):
        ...
        # 输出目录
        output_dir = r"C:\Users\xiao\Desktop\demo\ocr"
        os.makedirs(output_dir, exist_ok=True)

        # 文件名：业务大类-应用名称-ocr识别-时间戳.xlsx
        safe_category = self.category.replace("/", "-").replace("\\", "-") if self.category else "未知分类"
        safe_app = self.app_name.replace("/", "-").replace("\\", "-") if self.app_name else "未知应用"
        filename = f"{safe_category}-{safe_app}-ocr识别-{timestamp_str}.xlsx"
        self.excel_path = os.path.join(output_dir, filename)
