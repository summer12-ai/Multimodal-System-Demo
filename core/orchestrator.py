...
class MasterOrchestrator:
    """
    统一调度 OCR、Logcat、Traffic 三个子模块，输出融合状态与统一结果。
    Traffic 模块提供网络层 Ground Truth，用于评估 OCR/Logcat 准确率。
    """
    ...
    def __init__(self, cfg: OrchestratorConfig):
        ...
        self._last_accuracy_report_at: float = 0.0
        self._accuracy_report_interval: float = 30.0
        if cfg.enable_traffic:
            self.traffic_service = TrafficModuleService(
                target_app=cfg.target_app,
                device_id=cfg.device_id,
                package_name=cfg.traffic_package_name,
                enable_pcap=cfg.enable_traffic_pcap,
                window_seconds=cfg.traffic_window_seconds,
            )
        else:
    ...
    def _collect_recent_events(self, ocr_snapshot, log_snapshot, traffic_snapshot, fusion):
        ...
        if traffic_snapshot:
            tr_state = (traffic_snapshot.get("state") or {}).get("state", "N/A")
            tr_conf = float((traffic_snapshot.get("state") or {}).get("confidence", 0.0))
            events.append(f"TRAFFIC state={tr_state} conf={tr_conf:.2f}")
            acc = traffic_snapshot.get("accuracy") or {}
            if acc.get("ocr_match") is not None:
                events.append(
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                if traffic_snapshot:
                    # Ground Truth 评估
                    self.traffic_service.feed_ocr_log(ocr_snapshot, log_snapshot)
                    acc = self.traffic_service.evaluate()
                ...
                # 定期打印累计准确率报告
                if self.traffic_service and (time.time() - self._last_accuracy_report_at) >= self._accuracy_report_interval:
                    report = self.traffic_service.get_accuracy_report()
                    if report.get("status") == "ok":
                        ov = report.get("overall", {})
                        self._recent_events.append(
                            f"[ACC_REPORT] ocr_acc={ov.get('ocr_accuracy', 0):.2f} "
                            f"log_acc={ov.get('log_accuracy', 0):.2f} "
                            f"3way={ov.get('three_way_agreement', 0):.2f}"
                        )
