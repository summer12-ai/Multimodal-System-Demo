...
def render_dashboard(
    ...
    print("-" * 100)
    if traffic_snapshot:
        tr_state = traffic_snapshot.get("state") or {}
        tr_proto = traffic_snapshot.get("protocol") or {}
        tr_net = traffic_snapshot.get("net_stats") or {}
        tr_pkt = traffic_snapshot.get("packet_stats") or {}
        print("[Traffic 子模块]")
        rx_rate_val = tr_net.get('rx_rate', 0) or 0
        print(
            f"state={_safe(tr_state.get('state', 'UNKNOWN'))} | "
            f"conf={float(tr_state.get('confidence', 0.0)):.2f} | "
            f"rx_rate={rx_rate_val:.0f}B/s | "
            f"rssi={_safe(tr_net.get('rssi', 'N/A'))} | "
            f"conn={_safe(tr_net.get('active_connections', 0))}"
        )
        if tr_pkt.get("packets"):
            proto_str = ",".join(f"{k}:{v}" for k, v in (tr_pkt.get("proto_counts") or {}).items())
            print(
                f"packets={tr_pkt.get('packets', 0)} | "
                f"bytes={tr_pkt.get('total_bytes', 0)} | "
                f"protos={proto_str}"
            )
        if tr_proto.get("hls_resolution") or tr_proto.get("dash_resolution"):
            print(
                f"protocol={tr_proto.get('protocol', '')} | "
                f"res={tr_proto.get('hls_resolution') or tr_proto.get('dash_resolution')} | "
                f"bitrate={tr_proto.get('hls_bandwidth') or tr_proto.get('dash_bandwidth', 0)}bps"
            )
        acc = traffic_snapshot.get("accuracy") or {}
        if acc.get("ocr_match") is not None:
            print(
                f"[GT评估] ocr_match={acc.get('ocr_match')} | "
                f"log_match={acc.get('log_match')} | "
